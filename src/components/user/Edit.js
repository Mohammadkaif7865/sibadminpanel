...
function UserEdit() {
      ...
      const res = await axios.get(`${API_URL}user/single/${id}`, {
        ...
        if(data.user) {
          setInputs({
            ...
            can_access: data.user[0].can_access ? data.user[0].can_access.split(',') : [],
          });
        }
    };
    useEffect(() => {
    ...
    return (
      <Layout>
        ...
        <h4 className="fw-bold py-3 mb-4"><span className="text-muted fw-light"></span> User Edit</h4>
        <div className="row">
                  ...
                  <div className="mb-3">
                    ...
                    {modules &&
                      modules.map((module, index) => (
                        <><input key={index} type="checkbox" name="can_access" checked={inputs.can_access.includes(module.value) ? 'checked' : ''} onChange={handleInputChange} value={module.value} /> {module.name} <br /></>
                    ))}
                  </div>
